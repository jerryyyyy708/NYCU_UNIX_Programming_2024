...
int nbp = 0;
struct breakpoint *sorted_breakpoints[MB];

int compare(const void *a, const void *b) {
    struct breakpoint *bp_a = *(struct breakpoint **)a;
    struct breakpoint *bp_b = *(struct breakpoint **)b;
    return (bp_b->addr > bp_a->addr) - (bp_b->addr < bp_a->addr);
}
...
void reset_bp() {
    // 初始化指標數組
    for (int i = 0; i < MB; i++) {
        sorted_breakpoints[i] = &breakpoints[i];
    }

    // 使用 qsort 對指標數組進行排序
    qsort(sorted_breakpoints, MB, sizeof(struct breakpoint *), compare);

    // 逆序遍歷排序後的指標數組，從小到大重設斷點
    for (int i = MB - 1; i >= 0; i--) {
        if (sorted_breakpoints[i]->enabled) {
            // 使用 ptrace 恢復原始指令
            if (ptrace(PTRACE_POKETEXT, child_pid, (void*)sorted_breakpoints[i]->addr,
                       (void*)sorted_breakpoints[i]->original_data) != 0) {
...
void set_bp() {
    ...
    for (int i = 0; i < MB; i++) {
        if (sorted_breakpoints[i]->enabled) {
            long int3_instruction = (sorted_breakpoints[i]->original_data & ~0xFF) | 0xCC;
            // 使用 ptrace 設置 int3 斷點
            if (ptrace(PTRACE_POKETEXT, child_pid, (void*)sorted_breakpoints[i]->addr,
                       (void*)int3_instruction) != 0) {
                perror("Failed to set breakpoint");
            }
